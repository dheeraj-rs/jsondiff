...
import { FileJson, ArrowLeftRight, ArrowLeft, Clipboard, Trash, Code } from "lucide-react";
...
const JsonCompare = () => {
  ...
  const handlePaste = async (side: "left" | "right") => {
    try {
      ...
      const formattedJson = autoConvertToJson(text);
      if (side === "left") {
        setLeftJson(formattedJson);
      } else {
        setRightJson(formattedJson);
      }
      toast({
      ...
      toast({
        ...
        description: "Please paste valid JSON manually",
        variant: "destructive",
      });
    }
  };

  const handleClearInput = (side: "left" | "right") => {
    if (side === "left") {
      setLeftJson("");
    } else {
      setRightJson("");
    ...
    toast({
      title: "Input cleared",
      description: `${side} JSON input has been cleared`,
    });
  };
  ...
  const handleFormatJson = (side: "left" | "right") => {
    try {
      ...
      toast({
        ...
        description: `Formatted ${side} JSON`
      });
    } catch (err) {
      toast({
        title: "Invalid JSON",
  ...
  const autoConvertToJson = (text: string): string => {
    try {
      // Try to parse as JSON first
      return JSON.stringify(JSON.parse(text), null, 2);
    } catch (error) {
      // If parsing fails, convert to JSON-like format
      try {
        const jsonLike = `{"content": ${JSON.stringify(text)}}`;
              ...
              <Card className="p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => handleFormatJson("left")}>
                      <Code className="h-4 w-4 mr-2" />
                      Format
                    </Button>
                    <Button variant="ghost" size="sm" onClick={() => handleClearInput("left")}>
                ...
                <div className="relative">
                  <textarea
                    ...
                    className="w-full h-[400px] p-2 font-mono text-sm border rounded-md resize-none focus:outline-none focus:ring-2 pl-10"
                    placeholder="Paste your JSON here..."
                  />
                  <div className="absolute left-2 top-2 bottom-2 w-8 text-gray-400 font-mono text-sm overflow-y-auto">
                    {leftJson.split("\n").map((_, i) => (
              ...
              <Card className="p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => handleFormatJson("right")}>
                      <Code className="h-4 w-4 mr-2" />
                      Format
                    </Button>
                    <Button variant="ghost" size="sm" onClick={() => handleClearInput("right")}>
                      <Trash className="h-4 w-4" />
                    </Button>
                  </div>
                </div>
